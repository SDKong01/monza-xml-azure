...
class ClickHouseOBTManager(BaseManager):
    ...
    def _distinct(self):
        self.query_string = f"SELECT DISTINCT {self.query.distinct_field} FROM {self.query.schema}.{self.query.table} LIMIT 500;"

    def _process_pivots(self):
        pivots = []
        for p in self.query.pivot or []:
            query_fields = f"SELECT DISTINCT {p['pivot']} FROM {self.query.schema}.{self.query.table} LIMIT 500;"
    ...
    def _process_fields(self):
        ...
        for f in self.query.fields:
            ...
            field_name = parse_field_name(f.field, f.field)
            if op:
    ...
    def build_query_string(self, query: Query = None) -> str:
        self.query = query
        if self.query.distinct_field:
            self._distinct()
            return
        ...
        query_string = f"SELECT {self.fields} FROM {self.query.schema}.{self.query.table} {self.table_alias} {self._filters} {self._group_by} {self._order_by} {self._limit}{self.finish_query}"
